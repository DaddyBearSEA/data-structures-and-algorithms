...
const isNum = (input) => {
  // Solution code here...
  let numberInput = /[0-9]/g;
  input = String(input);
  if (input.match(numberInput)) {
    return true;
  }else {
    return false;
  }
...
const isCapitalized = (str) => {
  // Solution code here...
  let capitalInput = /[A-Z]\w+/g;
  if (str.match(capitalInput)) {
    return str.match(capitalInput);
  }else {
    return [];
  }
};
...
Write a function named citiesAtoJ that takes in an array of city names and uses a regular expression pattern to return a new array containing anycities that begin with the letters A through J, inclusive.
------------------------------------------------------------------------------------------------ */
