...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach(profile => {
    let $profile = $template.clone();
    $profile.find('h2').text(profile.name);
    $profile.find('h3').text(profile.height);
    $profile.find('p').text(profile.eye_color);
    $('main').append($profile);
  });
};
...
const howMuchPencil = (str) => {
  ...
  for (let i = 0; i < str.length+1; i++) {
    let newWord = str.slice(i, str.length);
    result.push(newWord);
  }
  return result;
};
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(value => {
    let outputIng = value.slice(((value.indexOf(' ', 3)) + 1), value.length);
    result.push(outputIng);
