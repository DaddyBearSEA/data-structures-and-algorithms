...
const toLastNames = people => {
  // Solution code here...
  let names = people.map((x)=> x.firstName + ' ' + x.lastName);  // fun fun functions guy MPJ Rocks!
  return names;
};
...
const validatePin = (pin) => {
  // Solution code here...
  let checkPin = /^[0-9]{4}$/;
  let pinCheck = checkPin.test(pin);
  return pinCheck;
};
...
const validateEmail = (email) => {
  ...
  let checkEmail = /^\w+.\w+@\w+.?(com|net|org)$/;
  // let emailCheck = checkEmail.test(email);
...
const validatePhoneNumber = (phoneNumber) => {
  ...
  let checkPhone = /(^\(\d{3}\)|(^\d{3}))[ -]?\d{3}[ -]?\d{4}$/g;
  let phoneCheck = checkPhone.test(phoneNumber);
  return phoneCheck;
